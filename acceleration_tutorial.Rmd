...
By examining the output of the `plot` function, we can observe the variations in transition probabilities for each minute of the day. These are the same for both models given the mentioned above.
...
Examining the pseudo-residual time series, qq-plots, and autocorrelation functions (ACF), we note very similar outcomes to the model with no covariates. This similarity is primarily attributed to the fact that the parameters associated with the state-dependent distributions remained virtually unchanged.

```{r, fig.height= 4, fig.width=12}
plotPR(cfit2)
```

We can also plot the stationary state probabilities. These represent how likely the shark is to be at each behavioural state for each minute of the day. According to the plot below, the shark tends to be in the  
